...
{ 
	...
	flashmark=FlashRead(0x080E0000);
	if(flashmark != 2)//判断标识符，写入校准数据，建议在imu温度闭环后再写入
	 {
		  FlashErase11();
		  FlashWrite(0x080E0000,2);
	    FlashWrite(0x080E1000,mpu_data.ax_offset);
	    FlashWrite(0x080E2000,mpu_data.ay_offset);
	    FlashWrite(0x080E3000,mpu_data.az_offset);
	    FlashWrite(0x080E4000,mpu_data.gx_offset);
	    FlashWrite(0x080E5000,mpu_data.gy_offset);
		  FlashWrite(0x080E6000,mpu_data.gz_offset);
   }
	else
	 {
		  flashread[0] = FlashRead(0x080E1000);//读取校准值，如果跟经验值误差过大，认为校准有误，使用经验值
		  flashread[1] = FlashRead(0x080E2000);
		  flashread[2] = FlashRead(0x080E3000);
		  flashread[3] = FlashRead(0x080E4000);
		  flashread[4] = FlashRead(0x080E5000);
		  flashread[5] = FlashRead(0x080E6000);
		  if(Func_Abs(mpu_data.ax_offset - flashread[0]) > 3)
		    mpu_data.ax_offset=flashread[0];
		  if(Func_Abs(mpu_data.ay_offset - flashread[1]) > 3)
		    mpu_data.ay_offset=flashread[1];
			if(Func_Abs(mpu_data.az_offset - flashread[2]) > 3)
		    mpu_data.az_offset=flashread[2];
			if(Func_Abs(mpu_data.gx_offset - flashread[3]) > 3)
		    mpu_data.gx_offset=flashread[3];
			if(Func_Abs(mpu_data.gy_offset - flashread[4]) > 3)
		    mpu_data.gy_offset=flashread[4];
			if(Func_Abs(mpu_data.gz_offset - flashread[5]) > 3)
		    mpu_data.gz_offset=flashread[5];
	 }
}
...
{
	/* yaw    -pi----pi */
	imu.yaw = (int)(8192 + atan2(2*q1*q2 + 2*q0*q3, -2*q2*q2 - 2*q3*q3 + 1) * 1303.79f) % 8192; 
	/* pitch  -pi/2----pi/2 */
	imu.pit = ((-asin(-2*q1*q3 + 2*q0*q2)* 57.32464f) * 22.7555556f);   
	/* roll   -pi----pi  */	
